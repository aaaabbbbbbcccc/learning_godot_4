...
@export_range(128, 4096, 1, "exp") var dimension: int = 2048
...
func prepare_image() -> Image:
    ...
    var clone: Image = Image.new()
    clone.copy_from(heightmap)
    clone.resize(512, 512, Image.INTERPOLATE_NEAREST)
    var clone_tex: ImageTexture = ImageTexture.create_from_image(clone)
...
func compute_island_gpu(heightmap: Image) -> void:
    ...
    var workgroup_x: int = ceil(float(po2_dimensions) / 8.0)
    var workgroup_y: int = ceil(float(po2_dimensions) / 8.0)
...
func compute_island_cpu(heightmap: Image) -> void:
    var center: Vector2i = Vector2i(po2_dimensions, po2_dimensions) / 2
    for y: int in range(0, po2_dimensions):
        for x: int in range(0, po2_dimensions):
            var coord: Vector2i = Vector2i(x, y)
            var pixel: Color = heightmap.get_pixelv(coord)
            var distance: float = Vector2(center).distance_to(Vector2(coord))
            var gradient_color: Color = gradient.sample(distance / float(center.x))
...
func display_island(island: Image) -> void:
    var island_tex: ImageTexture = ImageTexture.create_from_image(island)
